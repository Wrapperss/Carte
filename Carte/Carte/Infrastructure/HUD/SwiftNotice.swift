...
private let sn_topBar: Int = 1_001
...
class SwiftNotice: NSObject {
    ...
    static let _rootViewController = UIViewController()
    ...
    static func wait(_ imageNames: Array<UIImage> = Array<UIImage>(), timeInterval: Int = 0) {
        ...
        //        window.frame = frame
        window.frame = UIScreen.screenBounds
    ...
    static func showNoticeWithText(_ type: NoticeType, text: String, autoClear: Bool, autoClearTime: Int) {
        ...
        var frame = CGRect(x: 0, y: 0, width: 150, height: 100)
        let window = UIWindow()
        window.rootViewController = _rootViewController
        window.backgroundColor = UIColor.clear
        let mainView = UIView()
        ...
        checkmarkView.contentMode = UIViewContentMode.center
        ...
        let label = UILabel()
        label.numberOfLines = 0
        label.preferredMaxLayoutWidth = Adaptor.widthFitting(300)
        label.font = UIFont.systemFont(ofSize: 13)
        label.textColor = UIColor.white
        label.text = text
        label.textAlignment = NSTextAlignment.center
        ...
        let lsize = label.systemLayoutSizeFitting(UILayoutFittingCompressedSize)
        let preferWidth = (lsize.width + 20 > frame.width) ? lsize.width + 20 : frame.width
        let preferHeight = (lsize.height + 62 + 15 > frame.height) ? lsize.height + 62 + 15 : frame.height
        frame = CGRect(origin: .zero, size: CGSize(width: preferWidth, height: preferHeight))
...
class SwiftNoticeSDK {
    ...
    class var imageOfDeletion: UIImage {
        ...
        UIGraphicsBeginImageContextWithOptions(CGSize(width: 26, height: 28), false, 0)
