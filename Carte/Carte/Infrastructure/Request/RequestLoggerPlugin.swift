...
public final class RequestLoggerPlugin: PluginType {
    ...
    public init(verbose: Bool = false, cURL: Bool = false, output: @escaping (_ separator: String, _ terminator: String, _ items: Any...) -> Void = RequestLoggerPlugin.reversedPrint, responseDataFormatter: ((Data) -> (Data))? = JSONResponseDataFormatter) {
        self.cURL = cURL
...
private extension RequestLoggerPlugin {
    ...
    func logNetworkRequest(_ request: URLRequest?) -> [String] {
        ...
        var flag = String(repeatElement("*", count: 50))
        flag = "\n\(flag) [ request ] \(flag)\n"
        var output = [flag]
        
        if let headers = request?.allHTTPHeaderFields {
            output += [format(loggerId, date: date, identifier: "HTTP Header", message: headers["Authorization"] ?? "No Authorization")]
        }
        
        if let bodyStream = request?.httpBodyStream {
            output += [format(loggerId, date: date, identifier: "Body Stream", message: bodyStream.description)]
        }
